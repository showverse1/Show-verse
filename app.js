...
// NOTE: Hardcoded AI-generated demo shows have been removed. Only real videos from Firestore collection 'showverse_episodes' are displayed.
...
export function initShowverseEpisodesSync() {
  // Purge any legacy mock AI demo videos from local cache and continue watching
  try {
    const cached = localStorage.getItem('showverse_cached_episodes');
    if (cached) {
      const parsed = JSON.parse(cached);
      if (Array.isArray(parsed) && parsed.length > 0) {
        // Keep ONLY real Firestore items (they have docId or isLiveFirestore or were uploaded to Firestore)
        const realOnly = parsed.filter(ep => ep.isLiveFirestore === true || ep.docId || ep.uploadedAt);
        firestoreEpisodes = realOnly;
      }
    }
    // Also clean continue watching and watch history from old ocean demo videos
